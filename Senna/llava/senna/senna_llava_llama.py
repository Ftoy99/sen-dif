'''
Senna VLM model, modified from LLaVA.
'''
...
from Senna.llava.senna.senna_llava_arch import LlavaMetaModel, LlavaMetaForCausalLM
from Senna.llava.constants import IGNORE_INDEX, IMAGE_TOKEN_INDEX
...
class SennaLlavaLlamaForCausalLM(LlamaForCausalLM, LlavaMetaForCausalLM):
    config_class = LlavaConfig
    ...
    def mm_project(self, image_features):
        new_image_features = []
        ...
        for i in range(batch):
            return_image_features.extend([new_image_features[j][i] for j in range(num_img)])
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        # image_features = rearrange(image_features, f'{B*N} M D -> {B} {N} M D')
...
AutoModelForCausalLM.register(LlavaConfig, SennaLlavaLlamaForCausalLM)
